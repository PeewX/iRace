...
function CToptimes:openWindow ()
	...
	-- windowbg as parent:
		...
		-- container as parent:
			...
			self.gui['title0'] = guiCreateLabel(0, 0, sizeX, 30, 'Hunter times', false, self.gui['windowbg'] )
			guiLabelSetHorizontalAlign ( self.gui['title0'], 'center' )
			guiSetFont(self.gui['title0'], 'default-bold-small')
			guiLabelSetColor ( self.gui['title0'],255,26,00)
			...
			guiLabelSetColor ( self.gui['title'], 255, 2, 5, 255 )
			...
			guiLabelSetColor ( self.gui['header'], 255, 12, 192 )
			...
			guiLabelSetColor ( self.gui['headerul'], 255, 192, 252 )
...
function CToptimes:doToggleToptimes( bOn )
	...
	if self.bManualShow and not self.hasTimes then
		self.bManualShow = false
	end

	-- Set auto off timer if switching on
	if self.bManualShow then

		self.autoOffTimer:setTimer( function() self:doToggleToptimes(false) end, 10000, 1 )
