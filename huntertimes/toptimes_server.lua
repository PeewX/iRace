...
addEventHandler('onPlayerPickUpRacePickup', g_Root,
	function(number, sort, model,target)
...
function SToptimesManager:playerFinished( player, newTime, dateRecorded )
	...
	--triggerEvent ( "onToptimeGotEvent", player, newPos, oldPos )
	...
	function( player, cmd, place )
		...
		if g_SToptimesManager and g_SToptimesManager.mapTimes then
			...
			if row then
				...
				--outputChatBox( "Top time"..placeText.." von '" .. tostring(row.playerName) .. "' wurde von " .. getPlayerName(player).." gelöscht", 255,55,00 )
