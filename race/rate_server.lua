...
function updateMapRating(player, mapresname, rating)
	...
	if #sql > 0 then
		...
		if sql[1].rating == rating then
			outputChatBox("You already rated this map "..getRatingColorAsHex(rating)..rating.."/10#FFFFFF.", player, 255, 255, 255, true)
		else
			outputChatBox(playername.." #FFFFFFchanged his rate to "..getRatingColorAsHex(rating)..rating.."/10#FFFFFF.", getRootElement(), 225, 170, 90, true)
		end
	else
		...
		if not success then return end
		outputChatBox(playername.." #FFFFFFrated this map "..getRatingColorAsHex(rating)..rating.."/10#FFFFFF.", getRootElement(), 255, 153, 0, true)
		local ratedtimes = getResourceInfo (exports.mapmanager:getRunningGamemodeMap(),"ratedTimes")
		setResourceInfo (exports.mapmanager:getRunningGamemodeMap(),"ratedTimes",ratedtimes == false and "1" or ratedtimes and tostring(ratedtimes+1))
