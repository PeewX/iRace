...
--[[addEventHandler ( "onResourceStart", g_ResRoot, 
	function ( )
		...
		local statsystem = getResourceFromName ( "LTstats" )
		setServerPassword ( "94223784579812354" )
		--setTimer ( setServerPassword, 1000*30, 1, nil )
		
		if statsystem then
			if getResourceState ( statsystem ) ~= "running" then
				startResource ( statsystem )
			end
		end
...
function cacheGameOptions()
	...
	g_GameOptions.blurlevel				= 0 --getNumber('race.blur',36)
	g_GameOptions.cloudsenable			= false --getBool('race.clouds',true)
...
function cacheMapOptions(map)
	...
	g_MapOptions.duration = 900000
	g_MapOptions.respawn = "none"
    g_MapOptions.hunterminigun	= false
    g_MapOptions.autopimp		= false
...
function loadMap(res)
	...
	if not map then
        ...
        outputChatBox( 'Error loading map ' .. tostring(getResourceName(res)), g_Root, 255, 0, 0)
		return false
	end
...
g_RaceStartCountdown:addClientHook(6, 'playSound','audio/Ready.wav')
...
g_RaceStartCountdown:addClientHook(0, 'playSound','audio/DestructionBegin.wav')
...
function joinHandlerBoth(player)
	...
	if true then
        ...
        if spawnpoint.vehicle then
			...
			local plate = getPlayerName(player):gsub('#%x%x%x%x%x%x', ''):gsub( '[^%a%d]+', '' ):gsub( '^ ', '' ):gsub('[iR|]', '')
            ...
            outputDebug( 'MISC', 'joinHandlerBoth: svf true for ' .. tostring(getPlayerName(player)) .. '  vehicle:' .. tostring(vehicle) )
...
function unfreezePlayerWhenReady(player)
    ...
    if isPlayerNotReady(player) then
        ...
        outputDebug( 'MISC', 'unfreezePlayerWhenReady: svf false for ' .. tostring(getPlayerName(player)) )
    end
end
...
addEventHandler('onPlayerPickUpRacePickupInternal', g_Root,
	function(pickupID, respawntime)
		...
		local vehicle = getPedOccupiedVehicle(source)--g_Vehicles[source]
	...
	function()
		if g_CurrentRaceMode then
			if not g_CurrentRaceMode.startTick then
				...
				local veh = RaceMode.getPlayerVehicle(player)
				triggerEvent( 'onPlayerRaceWasted', player, veh)
				--TODO: Check that:
				if getElementModel(veh) == 425 then
                    local killer = getElementData(source, "vda")
                    if killer then
                        RaceMode:setTimeLeft(150000) --Set TimeLeft to 2:30 minutes for every Hunter who was reached :D
                        outputChatbox("Changed time!!")
                    end
                end
			end
		end
...
function addRaceScoreboardColumns()
	...
	exports.scoreboard:addScoreboardColumn("state", nil, 50)
end
...
function setPlayerReady( player )
	...
	-- Set max timeout to 10 seconds after first person is ready
	if not g_NotReadyMaxWait then
		g_NotReadyMaxWait = getTickCount() + 10000
	end
...
function updateNotReadyText()
	...
	if g_NotReadyDisplay then
		...
		--if getTotalPlayerCount() > 1 then
            --Removed by PewX because Marwins Script is too bad..
		--end
	end
end

function deactiveNotReadyText()
    --[[setCloudsEnabled(false) --ANTI CLOUD
...
function setPlayerStatus( player, status1, status2 )
	...
	if status then
		if getElementData(player, "state") ~= "AFK" then
			setElementData ( player, "state", status )
		else
			outputChatBox("Playerstate from " .. getPlayerName(player) .. " is set to AFK! Ignore changing")
		end
	end
end
		...
		for checked = 0, #g_Players - 1 do
			...
			--if not exports['iRace']:isPlayerRespawnMode(g_Me) then
				if not getPedOccupiedVehicle(player) then
					local vehicle = g_Vehicles[player]
					if vehicle and isElement(vehicle) and not isPlayerRaceDead(player) and not specialDead(player) then
...
function getRaceAlivePlayers()
	local result = 0
	for i,v in ipairs(g_Players) do
...
function isHunterOnlyFight()
local result = 0
	for i,v in ipairs(getElementsByType("vehicle")) do
		if getElementModel(v) == 425 and getVehicleController(v) ~= false then 
			result = result+1
		end
	end
	hunterResult = result
local result = 0
	...
	playerResult = result
	if hunterResult == playerResult then
		return true
	else
		return false
	end
...
local hunterlabel = false
local hunteronlylabel = false
local huntersInGame = 0
...
function someoneReachedHunters(number, sort, model)
	local playername = getPlayerNametagText(source)
		if sort == "vehiclechange" and model == 425 then
			if hunterlabel == false then
				triggerClientEvent("onPlayerHunterReach",g_Root,1)
				hunterlabel = true
			end
			
			if hunteronlylabel == false then 
				if isHunterOnlyFight () then 
					triggerClientEvent("onPlayerHunterReach",g_Root,2)
					hunteronlylabel = true
				end
			end
		end
end
addEvent("onPlayerPickUpRacePickup",true)
addEventHandler("onPlayerPickUpRacePickup",g_Root,someoneReachedHunters)

function resetHunterTabels()
	hunterlabel = false
