...
function RankingBoard:add(player, time)
	...
	if table.find(self.items, 'name', player) then
		return
	end
	table.insert(self.items, { name = player, time = time })
	self:clientCall(g_Root, 'add', player, time)
end

function quitData ()
triggerClientEvent("addQuitPlayerToGUI",getRootElement(),getPlayerNametagText(source))
end
addEventHandler("onPlayerQuit",getRootElement(),quitData)

function joinData ()
triggerClientEvent("addJoinPlayerToGUI",getRootElement(),getPlayerNametagText(source))
end
addEventHandler("onPlayerJoin",getRootElement(),joinData)
