...
function showGUIComponents(...)
	for i,name in ipairs({...}) do
		if name == "timeleftbg" then outputChatBox("show timeleftbg", 0, 0, 255) end
...
function hideGUIComponents(...)
	for i,name in ipairs({...}) do
		if name == "timeleftbg" then outputChatBox("Hide timeleftbg", 255, 0, 0) end
		if g_dxGUI[name] then
...
function checkVehicleIsHelicopter()
	local vehID = getElementModel(getPedOccupiedVehicle(g_Me))
	if vehID == 417 or vehID == 425 or vehID == 447 or vehID == 465 or vehID == 469 or vehID == 487 or vehID == 488 or vehID == 497 or vehID == 501 or vehID == 548 or vehID == 563 then
		setHelicopterRotorSpeed (getPedOccupiedVehicle(g_Me), 0.2)
	end
end
...
function fadeCamera(fadeIn,timeToFade,...)
    if not fadeIn and not getElementData(g_Me, "isLogedIn") then return end
