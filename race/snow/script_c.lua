...
sx2,sy2 = sx/2,sy/2
local settings = {type = "real", density = 500, wind_direction = {0.01,0.01}, wind_speed = 0.1, snowflake_min_size = 2, snowflake_max_size = 3, fall_speed_min = 1, fall_speed_max = 2, jitter = true}
...
function setSnowState(state)
	if state == true and snowState == false then
		snowState = true
		startSnow()
...
addEvent("setSnowState", true)
addEventHandler("setSnowState", getRootElement(), setSnowState)
