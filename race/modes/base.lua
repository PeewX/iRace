...
function RaceMode.isMapRespawn()
	return RaceMode.getMapOption('respawn') == 'none'
end
...
function RaceMode.endMap()
    if stateAllowsPostFinish() then
		...
		setTimer(RaceMode.startNextMapSelect, 5000, 1)
		triggerClientEvent("drawNextMapCountdown", getRootElement())
		triggerEvent('onPostFinish', g_Root)
    end
end
addEvent("startNewMap")
addEventHandler("startNewMap", getRootElement(), RaceMode.endMap)
...
function RaceMode:onPlayerReachCheckpoint(player, checkpointNum)
	...
	if checkpointNum < RaceMode.getNumberOfCheckpoints() then
		...
		self.rankingBoard:add(getPlayerNametagText(player), time,player)
...
function restorePlayer(id, player, bNoFade, bDontFix)
	...
	if vehicle then
        ...
        outputDebug( 'MISC', 'restorePlayer: svf true for ' .. tostring(getPlayerName(player)) .. '  vehicle:' .. tostring(vehicle) )
